...
export function CategoryModal({
  ...
  const [imageId, setImageId] = useState(initialCategory?.imageId ?? "");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!imageFile && !imageId.trim()) {
        setUploadError("Choose a category image first.");
        return;
      }
    ...
    >
      <div className="w-full max-w-[520px] overflow-hidden rounded-lg border border-[#EFE7D8] bg-white shadow-[0_24px_70px_rgba(36,31,20,0.18)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#EFE7D8] px-6 py-5">
          <div>
            <p className="text-xs font-extrabold uppercase text-[#34A85B]">Category details</p>
            <h2 id="category-modal-title" className="mt-1 text-2xl font-extrabold text-[#241F14]">
        ...
        <form className="grid gap-4 px-6 py-5" onSubmit={submit}>
          <label className="grid gap-2">
            <span className="text-xs font-bold text-[#8A8172]">Category Name</span>
            <input
  ...
  return (
    <div className="grid gap-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-bold text-[#8A8172]">Category image</span>
        {hasImage ? (
          <button
